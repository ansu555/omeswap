...
export default function FlowCanvas() {
  ...
  return (
    <div className="flex-1 relative bg-[#05050f] flex flex-col" onDragOver={onDragOver} onDrop={onDrop}>
        ...
        >
          ...
          <MiniMap
            ...
            nodeColor={(n) => {
              ...
              return cat === 'data' ? '#3b82f6' : cat === 'logic' ? '#eab308' : cat === 'action' ? '#22c55e' : '#a855f7'
            }}
            maskColor="#00000070"
          />
        </ReactFlow>
          ...
          {nodes.length === 0 && (
            ...
            >
              <div className="text-center pointer-events-auto">
                <p className="text-white/30 text-sm mb-1 font-medium">Start building your trading strategy</p>
                <p className="text-white/15 text-xs mb-8">or drag a node from the left panel</p>
                ...
                <div className="flex items-stretch gap-3">
                  ...
                  >
                    <LayoutTemplate size={22} className="text-purple-400/60 group-hover:text-purple-400 transition-colors" />
                    <div>
                      <p className="text-xs font-medium text-white/60 group-hover:text-white/80 transition-colors">Browse Templates</p>
                      <p className="text-[9px] text-white/20 mt-0.5">Pre-built strategies</p>
                    </div>
                  </button>
                  ...
                  <button
                    ...
                    className="flex flex-col items-center gap-3 px-6 py-5 rounded-2xl border border-purple-500/20 bg-purple-500/5 hover:bg-purple-500/10 hover:border-purple-500/35 transition-all group w-36"
                  >
                    <Bot size={22} className="text-purple-400/60 group-hover:text-purple-400 transition-colors" />
                    <div>
                      <p className="text-xs font-medium text-white/60 group-hover:text-white/80 transition-colors">AI Build</p>
                      <p className="text-[9px] text-white/20 mt-0.5">Describe your strategy</p>
                    </div>
                  </button>
                  ...
                  <button
                    ...
                    className="flex flex-col items-center gap-3 px-6 py-5 rounded-2xl border border-purple-500/20 bg-purple-500/5 hover:bg-purple-500/10 hover:border-purple-500/35 transition-all group w-36"
                  >
                    <Play size={22} className="text-purple-400/60 group-hover:text-purple-400 transition-colors" />
                    <div>
                      <p className="text-xs font-medium text-white/60 group-hover:text-white/80 transition-colors">Quick Start</p>
                      <p className="text-[9px] text-white/20 mt-0.5">Add Start node</p>
                    </div>
                  </button>
                </div>
              </div>
      ...
      {/* Bottom status bar */}
      <div className="h-7 shrink-0 border-t border-purple-500/10 bg-[#0d0d1a]/80 flex items-center px-4 gap-4 text-[10px] font-mono text-white/25">
        {/* Node + edge count */}
        <div className="flex items-center gap-1.5">
          <Circle size={6} className={clsx(botRunning ? 'text-green-400 animate-pulse' : 'text-white/20')} />
        ...
        {avaxExposure > 0 && (
          <>
            <span className="text-white/10">|</span>
            <span className={avaxExposure > 5 ? 'text-amber-400/70' : 'text-white/25'}>
